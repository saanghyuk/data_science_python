...
- #### 더블리 링크드 리스트 삭제 연산 개념
  ...
  싱글리 링크드 리스트에서는 삭제 연산에서 삭제하고 싶은 노드의 바로 앞 노드를 받아서, 그 다음 노드를 삭제 시켜주는 연산을 했음. 
  ...
  2. **head 노드를 지우려는 경우**(단 마지막 남은 노드는 아닌경우, 즉 1번 케이스는 아닌 경우)
