class GameCharacter:
    ...
    def get_attacked(self, damage):
        ...
        """
        if self.is_alive():
            self._hp = self._hp - damage if self._hp>= damage else 0
        else:
            print("{}님은 이미 죽었습니다.".format(self._name))

    def attack(self, other_character):
        # 게임 캐릭터가 살아있으면 파라미터로 받은 다른 캐릭터의 체력을 자신의 공격력만큼 깎는다.
        if self.is_alive():
            other_character.get_attacked(self._power)

    def __str__(self):
        # 게임 캐릭터의 의미있는 정보를 포함한 문자열을 리턴한다.
        return "{}님의 hp는 {}만큼 남았습니다.".format(self._name, self._hp)
